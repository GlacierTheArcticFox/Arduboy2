...
uint8_t Arduboy2Base::getPixel(uint8_t x, uint8_t y)
{
  ...
  return lcd.scr[y*(WIDTH/8)+x/8] & (1 << ((x % 8)*-1+7)) ? 1 : 0;
}
...
void Arduboy2Base::fillScreen(uint8_t color)
{
  ...
  memset(lcd.scr, color, WIDTH * HEIGHT / 8);
  /*
